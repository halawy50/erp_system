...
class EditMixProductionPage extends StatefulWidget {
  final MixProductionModel mixProductionModel;
  final MixProductionRepo mixProductionRepo;
  const EditMixProductionPage({super.key, required this.mixProductionRepo , required this.mixProductionModel});

  @override
  State<EditMixProductionPage> createState() => _EditMixProductionPageState();
}

class _EditMixProductionPageState extends State<EditMixProductionPage> {
  ...
  void initState() {
    ...
    _counter = widget.mixProductionModel.quantityMixProductions>0 ?
        widget.mixProductionModel.quantityMixProductions : 0 ; // نرجع العداد إلى 1 عند تغيير الخلطة
    _counterController = TextEditingController(text: '${_counter}');

    //date
    _dateController.text = widget.mixProductionModel.dateTimeProduction;
  ...
  Future<void> _selectDate(BuildContext context) async {
    DateTime defaultDate = DateFormat('yyyy-MM-dd').parse(widget.mixProductionModel.dateTimeProduction);

    DateTime? pickedDate = await showDatePicker(
      context: context,
      initialDate: defaultDate, // ← التاريخ المبدئي من النص
  ...
  Future<void> loadPrescriptions() async {
    ...
    itemsWarehouseManagement.add({
          'name': widget.mixProductionModel.nameMixProductions,
          'prescriptionId': widget.mixProductionModel.fkPrescription,

    });

    if (itemsWarehouseManagement.isNotEmpty) {
      selectedPrescription = itemsWarehouseManagement.first;
      int currentQuantity =widget.mixProductionModel.quantityMixProductions>0 ? widget.mixProductionModel.quantityMixProductions : 0 ;
      //اضافة عدد عمليات النتاج قبل التعديل + الاعداد المتوفرة للانتاج لهذه الخلطة
      maxPrescriptions = await widget.mixProductionRepo
          .getAvailablePrescription(selectedPrescription!['prescriptionId']) + currentQuantity;

      print("DataPrescription (Data : ${maxPrescriptions}) : ${maxPrescriptions}");
    }
  ...
  Future<void> onPrescriptionChanged(Map<String, dynamic>? value) async {
    ...
    int available = await widget.mixProductionRepo.getAvailablePrescription(prescriptionId);

    print("prescriptionQuantity : ${widget.mixProductionModel.quantityMixProductions}");
    setState(() {
                              ...
                              children: [
                                ...
                                DropdownButtonHideUnderline(
                                  child: DropdownButton2<Map<String, dynamic>>(
                                    ...
                                    value: selectedPrescription,
                                    onChanged: null, // هذا يجعل الـ dropdown غير قابل للتغيير
                                    buttonStyleData: ButtonStyleData(
                                      padding: const EdgeInsets.symmetric(horizontal: 0),
                                      height: 40,
                                      decoration: BoxDecoration(
                                        color: Colors.grey.shade200, // لون يوضح أنه غير مفعل
              ...
              child: Center(
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(
                        "هل تريد اتمام عملية تعديل الانتاج",
                        style: TextStyle(fontSize: 18),
                      ),
                      SizedBox(height: 20),
                      Row(
                              ...
                              onTap: (_counter >= 1 &&
                                ...
                                if(prescriptionName!=null &&
                                  ...
                                  MixProductionModel mixProductionModel = MixProductionModel(
                                      mixProductionsId :widget.mixProductionModel.mixProductionsId,
                                  ...
                                  if(widget.mixProductionModel.quantityMixProductions>0){
                                    bool isMixProductionInserted = await widget.mixProductionRepo.updateMixProduction(
                                        mixProductionModel ,
                                        widget.mixProductionModel.quantityMixProductions
                                    );
                                    if(isMixProductionInserted){
                                      print("تم تعديل الانتاج");
                                      showSnackbar(context, "تم تعديل الانتاج" ,backgroundColor: Colors.green);
                                      Navigator.pop(context);
                                    }else{
